...
| `edit [id]` | 在 `$VISUAL`/`$EDITOR` 中打开最近一条(或指定 ID 的)粘贴,**原地更新**该条目:ID、类型、标签均保持不变。HTML/RTF 编辑其源码且保持原类型,链接编辑 URL,颜色编辑 `#RRGGBBAA` 值。保存但未修改则不更新。与在应用内编辑一致,修改不会重新同步到已收到原条目的设备。 |
...
2. 目标设备会显示一个 6 位配对码——桌面和移动端显示在应用界面里，目标是 headless 守护进程时则通过 `crosspaste token` 查看；如有弹窗请在该设备上确认。
...
当被配对的目标设备没有屏幕——它运行的是 headless 守护进程——它的配对码不再通过弹窗显示，而是在那台机器上用 `crosspaste token` 查看：

```sh
# 在守护进程所在机器上（例如通过 SSH），配对前或配对中执行：
crosspaste token --wait

# 在另一台机器上照常发起配对（终端或应用界面均可），
# 然后输入守护进程机器上打印出的配对码。
crosspaste pair
```

配对码单独占一行输出到 stdout（脚本里可用 `CODE=$(crosspaste token)`），所有说明文字都走 stderr。不加 `--wait` 时，仅当已有配对尝试在进行中才会显示配对码，否则以退出码 1 结束。
