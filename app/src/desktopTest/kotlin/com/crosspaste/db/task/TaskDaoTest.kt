...
class TaskDaoTest {
    ...
    @Test
    fun `createTask returns valid task id`() =
        runTest {
            val taskId =
                taskDao.createTask(
                    pasteDataId = 100L,
    ...
    @Test
    fun `createTask with null pasteDataId succeeds`() =
        runTest {
            val taskId =
                taskDao.createTask(
                    pasteDataId = null,
    ...
    @Test
    fun `createTask stores correct task type`() =
        runTest {
            val taskId =
                taskDao.createTask(
                    pasteDataId = 1L,
    ...
    @Test
    fun `createTask sets initial status to PREPARING`() =
        runTest {
            val taskId =
                taskDao.createTask(
                    pasteDataId = 1L,
    ...
    @Test
    fun `createTask with custom extraInfo stores JSON`() =
        runTest {
    ...
    @Test
    fun `multiple createTask calls return unique ids`() =
        runTest {
    ...
    @Test
    fun `executingTask changes status to EXECUTING`() =
        runTest {
    ...
    @Test
    fun `successTask changes status to SUCCESS`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
    ...
    @Test
    fun `successTask with newExtraInfo updates extraInfo`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
    ...
    @Test
    fun `failureTask with needRetry resets status to PREPARING`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
    ...
    @Test
    fun `failureTask without needRetry sets status to FAILURE`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
    ...
    @Test
    fun `failureTask with newExtraInfo updates extraInfo`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
    ...
    @Test
    fun `executingTask updates modifyTime`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
    ...
    @Test
    fun `getTask returns null for non-existent id`() =
        runTest {
    ...
    @Test
    fun `getTask returns complete task data`() =
        runTest {
            val taskId =
                taskDao.createTask(
                    pasteDataId = 42L,
    ...
    @Test
    fun `cleanSuccessTask removes old success tasks`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
            taskDao.successTask(taskId, newExtraInfo = null)
    ...
    @Test
    fun `cleanSuccessTask does not remove tasks newer than threshold`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
            taskDao.successTask(taskId, newExtraInfo = null)
    ...
    @Test
    fun `cleanFailureTask removes old failure tasks`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
            taskDao.failureTask(taskId, needRetry = false, newExtraInfo = null)
    ...
    @Test
    fun `cleanFailureTask does not remove non-failure tasks`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
            taskDao.executingTask(taskId)
            taskDao.successTask(taskId, newExtraInfo = null)
    ...
    @Test
    fun `complete task lifecycle PREPARING to EXECUTING to SUCCESS`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
    ...
    @Test
    fun `task lifecycle with retry PREPARING to EXECUTING to PREPARING to EXECUTING to FAILURE`() =
        runTest {
            val taskId = taskDao.createTask(pasteDataId = 1L, taskType = TaskType.SYNC_PASTE_TASK)
