...
public func getPasteboardChangeCount(currentChangeCount: Int,
                                     isRemote: UnsafeMutablePointer<Int32>,
                                     isCrossPaste: UnsafeMutablePointer<Int32>,
                                     isConcealed: UnsafeMutablePointer<Int32>) -> Int {
    let pasteboard = NSPasteboard.general
    let newChangeCount = pasteboard.changeCount

    isRemote.pointee = 0
    isCrossPaste.pointee = 0
    isConcealed.pointee = 0
        ...
        for item in items {
            for type in item.types {
                if type.rawValue == "com.apple.is-remote-clipboard" {
                    ...
                    isCrossPaste.pointee = 1
                } else if type.rawValue == "org.nspasteboard.ConcealedType" ||
                            type.rawValue == "org.nspasteboard.TransientType" ||
                            type.rawValue == "org.nspasteboard.AutoGeneratedType" ||
                            type.rawValue == "com.agilebits.onepassword" {
                    // Password managers and private sources mark secrets/ephemeral data
                    // with these types (nspasteboard.org / 1Password);
                    // such content must never be recorded or synced.
                    isConcealed.pointee = 1
                }
            }
        }
    }
...
public func setAboutMenuItemTitle(title: UnsafePointer<CChar>) {
    ...
    DispatchQueue.main.async {
        ...
        guard let appMenu = NSApp.mainMenu?.items.first?.submenu,
              let aboutItem = appMenu.items.first(where: { !$0.isSeparatorItem }) else {
            return
        }
